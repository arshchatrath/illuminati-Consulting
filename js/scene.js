...
// Every layer shares one 1600x1000 coordinate space; main.js moves a
...
function layer(depth, name, body, defs = '') {
  return `<svg class="layer layer--${name}" data-depth="${depth}" xmlns="${NS}"
    viewBox="0 0 ${SCENE_W} ${SCENE_H}" width="${SCENE_W}" height="${SCENE_H}" overflow="visible" aria-hidden="true">
...
function sky() {
  ...
  while (x < 1300) {
    ...
    for (let wy = top + 16; wy < 640; wy += 20) {
      for (let wx = x + 9; wx < x + w - 12; wx += 15) {
        const lit = r();
        // Windows below the threshold start lit; the rest light up in chapter 4.
        city += `<rect class="win" data-t="${lit.toFixed(3)}" x="${wx}" y="${wy}" width="6" height="9"
          fill="${lit < 0.08 ? '#e3b04b' : '#11291f'}"/>`;
      }
    }
  ...
  return layer(0.55, 'sky', `
    ...
    <rect class="dawn" x="-600" y="250" width="2000" height="500" fill="url(#g-dawn)" opacity="0"/>
    <g class="city">${city}</g>`, defs);
}
...
function wall() {
  ...
  let cork = '';
  for (let i = 0; i < 260; i++) {
    cork += `<circle cx="${(696 + r() * 478).toFixed(0)}" cy="${(116 + r() * 328).toFixed(0)}" r="${(0.8 + r() * 1.4).toFixed(1)}" fill="#3a3121"/>`;
  }
  ...
  return layer(0.75, 'wall', `
    ...
    <ellipse class="spill" cx="1250" cy="560" rx="620" ry="380" fill="url(#g-spill)"/>
    <!-- window frame -->
...
function desk() {
  ...
  return layer(1, 'desk', `
    ...
    <g transform="rotate(-6 1060 890)">
      ...
      <path class="trend" d="M980 935 L1015 922 L1050 910 L1085 890 L1120 872 L1165 846" pathLength="1"/>
      <text x="1120" y="838" class="hand hand--sm trend-label" fill="#7a5e10">+4% topline</text>
    </g>
    <!-- laptop -->
    <g stroke="${INK}" stroke-width="1.5">
      <rect x="950" y="515" width="285" height="186" rx="8" fill="#0a1a14"/>
      <rect x="962" y="527" width="261" height="162" rx="3" fill="url(#g-screen)" class="screen"/>
    ...
    <g class="graph">${linkSvg}${nodeSvg}</g>
    <!-- puzzle -->
    ...
    <!-- lamp -->
    <path class="cone" d="M1268 528 L1362 520 L1460 900 L700 990 Z" fill="url(#g-cone)"/>
    <ellipse class="pool" cx="1040" cy="850" rx="480" ry="170" fill="url(#g-pool)"/>
    ...
    <ellipse class="bulb" cx="1314" cy="522" rx="70" ry="44" fill="url(#g-bulb)"/>`, defs);
}
...
function foreground() {
  return layer(1.25, 'fg', `
    ...
    <g class="rim" fill="none" stroke="#e8b85a" stroke-width="3" stroke-linecap="round">
...
export function buildScene(stage) {
  stage.innerHTML = sky() + wall() + desk() + foreground();
  return {
    ...
    notes: [...stage.querySelectorAll('.note')],
