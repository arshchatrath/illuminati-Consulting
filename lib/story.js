...
---------------------------------------------------------------- */
const CAM = {
  hero: [800, 500, 1.0], // scene centre: the whole illustration, fitted into the hero frame
  wide: [810, 560, 1.18],
  board: [935, 285, 2.05],
  puzzle: [770, 875, 2.1],
  laptop: [1092, 628, 2.3],
...
// Phones: a narrow frame can't use the desktop zooms. Each shot names the part of the scene
// that must stay in view, [centre x, centre y, width, height], and the camera fits that box
// into whatever shape the frame has. Landscape phones use the panorama (CAM_PHONE); portrait
// phones use the re-staged tall room (CAM_TALL), where everything sits in one column.
const TALL_QUERY = '(max-width: 819px) and (orientation: portrait)'; // matches the CSS in globals.css
const CAM_TALL = {
  hero: [800, 560, 820, 900], // board, desk, laptop, lamp and the consultant
  wide: [800, 300, 820, 1500], // the whole tall room: window and charts, board, desk
  board: [835, 280, 540, 372],
  puzzle: [985, 905, 420, 250], // both pairs, with the hand reaching in from the left
  laptop: [763, 612, 330, 250],
  city: [600, -183, 390, 420],
};
const CAM_PHONE = {
  hero: [790, 610, 1000, 700], // the consultant, the board, the laptop and the lamp
  wide: [820, 600, 720, 900], // between chapters: a step back, not the whole room
  board: [935, 280, 530, 372],
  puzzle: [735, 868, 470, 260],
  laptop: [1092, 612, 330, 250],
  city: [315, 330, 500, 540],
};

const KEYS = [
  { t: 0.0, f: 'hero', c: 'hero' },
  { t: 0.08, f: 'full', c: 'wide' },
  { t: 0.14, f: 'split', c: 'board' },
  { t: 0.26, f: 'split', c: 'board' },
  { t: 0.32, f: 'full', c: 'wide' },
  { t: 0.38, f: 'split', c: 'puzzle' },
  { t: 0.5, f: 'split', c: 'puzzle' },
  { t: 0.56, f: 'full', c: 'wide' },
  { t: 0.62, f: 'split', c: 'laptop' },
  { t: 0.74, f: 'split', c: 'laptop' },
  { t: 0.8, f: 'full', c: 'wide' },
  { t: 0.86, f: 'split', c: 'city' },
  { t: 1.0, f: 'split', c: 'city' },
];
...
export function initPage({ lamp = true } = {}) {
  ...
  // The staging on screen (wide panorama or tall phone room); re-bound if a phone is rotated.
  const tallQuery = matchMedia(TALL_QUERY);
  let scene = null;
  const bindActive = () => {
    const root = stage.querySelector(tallQuery.matches ? '.scene--tall' : '.scene--wide');
    if (scene?.root === root) return;
    scene = bindScene(root);
    Object.keys(varCache).forEach((k) => delete varCache[k]); // push every variable to the new scene
  };
  // Hero: on desktop the desk lamp's light follows the pointer across the desk.
  const cone = stage.querySelector('.scene--wide .cone');
  const pool = stage.querySelector('.scene--wide .pool');
  let aimTarget = 0;
  let aim = 0;
  ...
  let vh = 0;
  let mobile = false;
  let boxCam = false; // fit-the-box camera: phones, portrait or landscape
  let shapes = {};
  let fit = {}; // scene scale per shape: "hero" fits the whole scene, the others fill the screen
  ...
  function measure() {
    ...
    mobile = vw < 820;
    boxCam = mobile || vh < 500;
    const m = mobile ? 12 : 24;
    const navH = mobile ? 68 : 80;
    // Hero: the frame fills everything under the headline, and the scene is scaled to fit inside it,
    // so the whole illustration shows; the room's wall, desk and sky run on past its edges to fill the rest.
    const heroTop = hero.offsetTop + hero.offsetHeight + (mobile ? 18 : 12);
    // 1.06: a touch larger than an exact fit; the only thing trimmed is a strip of plain wall and floor.
    const heroFit = 1.06 * Math.min((vw - 2 * m) / 1600, (vh - heroTop - m) / 1000);
    caption.style.right = `${m + 18}px`;
    caption.style.bottom = `${m + 16}px`;
    ...
    const cover = Math.max(vw / 1600, vh / 1000) * 1.02;
    fit = { hero: heroFit, full: cover, split: cover };
    shapes = {
      hero: [heroTop, m, m, m],
      full: [navH, m, m, m],
      // Short landscape screens (phones on their side) give the text a wider column (matches the CSS).
      split: mobile ? [navH, m, textBand, m] : [navH, m, m, Math.round(vw * (vh < 500 ? 0.48 : 0.4))],
    };
  }

  // On phones the frame stays in its split shape between chapters (the camera pans instead),
  // so the text never blinks out; only the opening move goes full screen.
  const shapeOf = (key) => (mobile && key.f === 'full' && key.t > 0.1 ? 'split' : key.f);

  // Phone camera for one key: centre plus the scale that fits its box into that key's frame.
  function phoneCam(key) {
    const [top, right, bottom, left] = shapes[shapeOf(key)];
    const [cx, cy, bw, bh] = (tallQuery.matches ? CAM_TALL : CAM_PHONE)[key.c];
    return [cx, cy, Math.min((vw - left - right) / bw, (vh - top - bottom) / bh)];
  }
  ...
  function sampleKeys(t) {
    ...
    const sa = shapes[shapeOf(a)];
    const sb = shapes[shapeOf(b)];
    const inset = sa.map((v, j) => lerp(v, sb[j], k));
    if (boxCam) {
      const [ax, ay, as] = phoneCam(a);
      const [bx, by, bs] = phoneCam(b);
      // Scale moves geometrically so zooming in and out feel equally fast.
      return { inset, phone: [lerp(ax, bx, k), lerp(ay, by, k), as * Math.pow(bs / as, k)] };
    }
    return {
      inset,
      cam: CAM[a.c].map((v, j) => lerp(v, CAM[b.c][j], k)),
      base: lerp(fit[a.f], fit[b.f], k),
    };
  }
  ...
  function render(t) {
    const { inset, cam, base, phone } = sampleKeys(t);
    ...
    for (const { el, depth } of scene.layers) {
      ...
      let cy;
      if (phone) {
        // Near layers sit slightly larger than far ones, so pans still show parallax.
        [cx, cy] = phone;
        s = phone[2] * (1 + (depth - 1) * 0.08);
      } else {
        [cx, cy] = cam;
        s = base * (1 + (cam[2] - 1) * depth);
      }
    ...
    const heroOut = ease(band(t, 0, 0.045)); // gone before the growing frame reaches it
    ...
    let active = -1;
    // Phones: each chapter hands straight over to the next (no empty text band while the camera pans).
    const [fadeIn, fadeOut] = mobile ? [[0.06, 0.045], [0.045, 0.06]] : [[0.03, 0], [0, 0.03]];
    CHAPTERS.forEach(([a, b], i) => {
      const vis = band(t, a - fadeIn[0], a - fadeIn[1]) * (1 - band(t, b + fadeOut[0], b + fadeOut[1]));
      const el = chapters[i];
