...
export function initPage({ lamp = true } = {}) {
  ...
  function measure() {
    ...
    // Hero: the largest 16:10 box under the headline, so the whole illustration shows uncropped.
    const availTop = hero.offsetTop + hero.offsetHeight + (mobile ? 18 : 12);
    const availH = vh - availTop - m;
    const boxW = Math.max(0, Math.min(vw - 2 * m, availH * 1.6));
    const boxH = boxW / 1.6;
    const boxTop = availTop + (availH - boxH) / 2;
    const side = (vw - boxW) / 2;
    caption.style.right = `${side + 18}px`;
    caption.style.bottom = `${vh - boxTop - boxH + 16}px`;
    ...
    fit = { hero: boxW / 1600, full: cover, split: cover };
    shapes = {
      hero: [boxTop, side, vh - boxTop - boxH, side],
      full: [navH, m, m, m],
